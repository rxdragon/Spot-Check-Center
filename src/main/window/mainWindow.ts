...
export async function createWindow (): Promise<BrowserWindow> {
   ...
   */
  win.on('close', () => {
    (win as BrowserWindow).webContents.send('closed-win')
  ...
  win.on('enter-full-screen', () => {
    (win as BrowserWindow).webContents.send('enter-full')
  })
  ...
  win.on('leave-full-screen', () => {
    (win as BrowserWindow).webContents.send('leave-full')
  })
   ...
   */
  win.on('resize', () => {
    const data: number[] = (win as BrowserWindow).getSize()
    ;(win as BrowserWindow).webContents.send('win-resize', { data })
  })
 ...
 * @param {*} win 
 */
export async function cleanHttpCache (win: BrowserWindow) {
