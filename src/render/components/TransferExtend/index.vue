    ...
    <div v-loading="loading" class="transfer" :style="{width,height}">
      ...
      <div class="transfer-center">
        <p class="transfer-center-item">
          <el-button
            type="primary"
            :class="{'disable-box': from_disabled}"
...
export default defineComponent({
  ...
  computed: {
    ...
    to_data_people () {
      let peopleNum = 0
      this.self_to_data.forEach((group: any) => {
  ...
  watch: {
    ...
    from_check_keys (val) {
      if (val.length > 0) {
        ...
        const allCheck = val.filter((item: any) => item[this.pid] === 0)
  ...
  methods: {
    ...
    addToAims () {
      ...
      const newSkeletonHalfCheckedNodes: any[] = []
      skeletonHalfCheckedNodes.forEach((item: any) => {
      ...
      const newSkeletonCheckedNodes: any[] = []
      ...
      const leafCheckedNodes = arrayCheckedNodes.filter(
        (item: any) => !item[children__] || item[children__].length === 0
      )
      // 末端叶子插入目标树
      leafCheckedNodes.forEach((item: any) => {
        if (!inquireIsExist(item)) {
          this.$refs['to-tree'].append(item, item[pid__])
        }
      })
      // 递归查询data内是否存在item函数
      function inquireIsExist (item: any, strData = self_to_data) {
      ...
      // 左侧删掉选中数据
      arrayCheckedNodes.map((item: any) => this.$refs['from-tree'].remove(item))
      // 处理完毕按钮恢复禁用状态
    ...
    removeToSource () {
      ...
      // 递归查询data内是否存在item函数
      function inquireIsExist (item: any, strData = self_from_data) {
        // 将树形数据格式化成一维字符串 然后通过匹配来判断是否已存在
        const strItem
          = typeof item[id__] === 'number'
            ? `"${id__}":${item[id__]},`
      ...
      // 右侧删掉选中数据
      arrayCheckedNodes.forEach((item: any) => {
     ...
     * @param {*} treeObj
     */
    fromTreeChecked (nodeObj: any, treeObj: any) {
     ...
     * @param {*} treeObj
     */
    toTreeChecked (nodeObj: any, treeObj: any) {
     ...
     * @param {*} data
     */
    filterNodeFrom (value: any, data: any) {
     ...
     * @param {*} data
     */
    filterNodeTo (value: any, data: any) {
      if (!value) return true
      return data[this.defaultProps.label].indexOf(value) !== -1
    },
    /**
