...
function normalizeArray (parts: string[], allowAboveRoot: any) {
...
const substr = 'ab'.substr(-1) === 'b'
  ? function (str: string, start: number, len?: number) {
...
function normalize (path: string) {
  ...
  // Normalize the path
  path = normalizeArray(filter(path.split('/'), function (p: any) {
    return !!p
  }), !isAbsolutePath).join('/')
...
export function joinPath (dir: any, fileName: any) {
  // eslint-disable-next-line prefer-rest-params
  const paths = Array.prototype.slice.call(arguments, 0)
  const data = filter(paths, function (p: any, index: number) {
    if (typeof p !== 'string') {
 ...
 * @param {*} param0
 */
export function getFormat ({ dir , ext, name }: { dir: string , ext: string, name: string }) {
